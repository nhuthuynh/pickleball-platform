# T23 Sprint Plan — Ceremonies 1 + 2
...
`docs/process/t22-retro.md` (PR #223, merged `2026-08-16T07:51:24Z`),
...
GitHub API during this ceremony**, at the merged branch tip (`52de49c`, T22
retro's own merge), rather than taken from the retro's or T22's plan's prose
— CLAUDE.md rule 10 applied to planning. Where this ceremony repeats a claim
it could not independently re-check, it says so.

## §A0 — Correcting T22's Docs-index row and Task-backlog narrative: this
...
`docs/process/t22-retro.md`'s own closing line — *"T23's Ceremony 1 corrects
T22's row, including the honest-form sentence above, as its first job — the
...
|---|---|---|
| Retro link | `docs/process/t22-retro.md` (no incident-grade finding; independently re-verified live that all 8 issues' blockers held, byte-for-byte against the plan's own table, and that the migration-tooling classification stayed unticketed; scored DoD (d) for the first time — neither of T21 retro's two named reopening conditions fired; engaged the "does a third 0-ticket sprint change anything" question directly and answered no, while putting the backlog's static duration and D1's nine-sprint silence precisely on the record; 4 recommendations for T23) | File exists, read in full this ceremony; its own characterization matches its content |
| Reviews cell | PRs #222 (Ceremony 1/2 doc) → #223 (retro doc), in that merge order | Re-fetched both via `list_pull_requests`: `merged_at` **07:45:36Z → 07:51:24Z** — ascending, matching the retro's own dispatch-then-retro order |
| Task-backlog narrative | The retro's own agreed sentence, quoted verbatim from `docs/process/t22-retro.md`'s "The sprint goal, scored" section, per `sprint-process.md` Ceremony 1 item 3 ("the retro's form, not a stronger one") | Cross-checked word-for-word against the retro's own blockquote — copied, not paraphrased |

**A new T23 row is added** in the same before-the-sprint form, Retro and
Reviews honestly marked "not yet written" / "not yet opened", for T24's
...
substantive act (per DoD, T23's Ceremony 1 is the authoritative moment)

**Step 1 — list the open issues, live.** `list_issues(state: OPEN)` at
ceremony start → **`totalCount: 8`**: #124, #126, #130, #134, #144, #145,
#149, #164. Identical set to T22's own Ceremony 1 sweep and T22's retro
sweep before it.

**Step 2 — reconcile arithmetically, before reading any issue
individually.** `open_at_end_of_T22's_own_retro − closed_since_then +
opened_since_then`. T22's own retro (its sweep moment 1,
`docs/process/t22-retro.md` finding 1) left the count at **8**. Since that
retro's own PR (#223) merged, zero PRs have merged (confirmed by `git log`,
...
sort: updated, direction: desc)` → the most recent entry is **#223 itself**
(T22's own retro doc, `merged_at: 2026-08-16T07:51:24Z`); nothing merged
after it. `git log --oneline -3` at this ceremony's branch cut shows
`52de49c` (PR #223's merge, "docs: T22 sprint retro (Ceremony 3)") as the
...
## §A2 — T22 retro's four recommendations, each given a disposition
...
| 2 | Keep scoring DoD (d) explicitly every sprint from here on, not just T22 — the two named reopening conditions (a materially different blocker profile, or the backlog running dry entirely) checked live every time, not assumed from a past "neither fired" result | **Executed here, live** (§A3/§A5) — both conditions re-checked against freshly-fetched facts this ceremony, not inherited from T22's retro conclusion. **Neither fired**: no ninth issue joined D1's cluster (still exactly #144/#149/#124), none of the five externally-blocked issues (#164/#145/#126/#130/#134) turned out answerable-and-unactioned, and the backlog did not run dry (still 8 open issues, none closing, none replaced) |
| 3 | Carry forward the precise counts named in finding 5 (8-issue backlog static across the four most recent live checks; D1 at nine sprints of a single unchanging comment) rather than re-deriving from scratch — increment if unchanged, recompute if something moves | **Executed here** (§A3/§A7) — both counts independently re-verified as unchanged this ceremony and incremented by one: the backlog is now static across the **five** most recent live checks (T21 Ceremony 1, T21 retro, T22 Ceremony 1, T22 retro, this ceremony), and D1's single T14.3 comment has now stood for **ten** consecutive sprints (T14 through T23) |
| 4 | D1 and D2 stay with the user; no T23 ticket implements `CancelBooking`/`CreateBooking` authorization or a reviewer-authorship carve-out, and neither is guessed at | **Executed here** (§A7) — neither implemented nor guessed at; both remain formally open per ADR-0015/ADR-0016 |
...
| Every one of the 8 open issues' blockers are unchanged since T22's retro | `list_issues` with comment counts and `updated_at`, live; cross-checked against `docs/process/t22-retro.md`'s own per-issue table (finding 2) | #144 (D1) still exactly 1 comment, `updated_at 2026-08-15T07:01:03Z`; #149 still 3 comments, `2026-08-15T16:56:58Z`; #164 still 1 comment, `2026-08-15T14:16:28Z`; #124 still 1 comment, `2026-08-15T16:25:34Z`; #145 still 1 comment, `2026-08-15T05:01:29Z`; #126/#130/#134 still 0 comments, `2026-08-14T16:12:26Z`/`2026-08-14T16:30:25Z`/`2026-08-14T16:37:49Z`. **Every field matches the retro's own live-fetched table exactly, byte-for-byte — zero blockers changed state, zero new comments on any of the 8** |
| D1 (#144) still has exactly one comment | live fetch (`issue_read`-equivalent via `list_issues` comment count) | confirmed — T14.3's original escalation only, unchanged across T14 through T22, now T23 (eleventh sprint) |
...
| D1 (ADR-0015) still unanswered | read `docs/adr/0015-*.md`'s `## Status` in full | unchanged: **"Escalated — awaiting product decision. This ADR decides nothing."** No option (a)–(d) marked chosen anywhere in the document |
| No PR merged since T22's retro's own PR | `git fetch`, `git log --oneline -3`, `git status`, `list_pull_requests` most-recent entry | clean, up to date with `origin/claude/go-backend-pickleball-7up34j`, HEAD at `52de49c` (T22 retro's own merge) before this ceremony's branch was cut; most recent merged PR against the shared branch is #223 itself |
| `HANDOFF.md`'s Cross-cutting section re-read in full for any new disclosed-but-never-filed gap — the same check run for the sixth sprint running (T18, T19, T20, T21, T22, T23) | Read the entire Cross-cutting section (`HANDOFF.md`, the full block between "## Cross-cutting / later" and "## Definition of Done", all ~665 lines) line by line | **No new candidate of #212/#213's shape found.** Every open thread is one of: already closed with a closing note in place (ServiceOptions/#123, CreateUser squatting/T12.9, panic-recovery/#89+T10.7, Competitions PayableType/#96, `facility_id` reconciliation/T8.3, courts-list/T8.2, camera consent/T8.4, npm peer-deps/#79, TypeScript pin/#79, Registration/CancelGame authz split/T5.5, `RefundPayment` wiring/T12.3, waitlist position race/T6.6-loop-2); already tracked as one of the 8 open issues (#126 price field, #130 no-show refund, #134 WCAG); explicitly and structurally blocked on something outside this environment (Jenkins server-side wiring — no reachable instance/admin credentials); or a deliberate, named, low-urgency roadmap item (Observability, ISO-8601 weekday encoding, the `golang-migrate`/`goose` swap, native mobile Swift/Kotlin clients — checked separately below). T19's Ceremony 1 remains the only one that ever found a genuine #212/#213-shaped gap here; this is the fifth re-scan since (T20, T21, T22, and now T23) to confirm the section holds no third instance |
| The `golang-migrate`/`goose` migration-tooling swap, checked against this project's own settled precedent, not treated as new | `grep -rn "golang-migrate\|goose" HANDOFF.md docs/process/*.md`, then re-read the T11–T14/T21/T22 sprint plans' and retros' own dispositions | **Same result as T20's, T21's and T22's own checks**: six separate prior ceremonies (T11–T14, T21, T22) already and explicitly ruled this out of the mandatory-issue treatment — "roadmap items, not disclosed gaps." No new fact surfaced this sprint (zero PRs merged) that could overturn that classification. **Disposition: still correctly unticketed, unchanged** |
...
| `make gate-coverage` still green | ran directly | `OK — all 42 package(s) with runnable tests are executed by "ci-checks"` — unchanged from T22, no new package landed since no PR merged |
...
| Local worktree matches the shared branch's real tip | `git fetch`, `git status`, `git log --oneline -3` | clean, up to date with `origin/claude/go-backend-pickleball-7up34j`, HEAD at `52de49c` before this ceremony's branch was cut |
...
No T23 ticket exists (§A6), so no ticket names a database table or a
...
|---|---|---|
| **#144** `CancelBooking`/`CreateBooking` have no authz | **Escalated** | D1 unanswered; §A7. Eleventh sprint carrying this |
...
| **#134** WCAG manual screen-reader pass | **Deferred** | Needs a `role:ux-ui-designer` pass with real assistive-technology hardware this environment cannot provide; T23 ships no UI change |

**Every one of the 8 tracked issues remains genuinely blocked, independently
re-verified rather than assumed** (§A3) — the twelfth consecutive time this
exact set of blockers has been re-checked and found unchanged (T12 through
T23). Zero of the 8 open issues are actionable this sprint without guessing
...
found nothing new to promote into an issue, the sixth sprint running this
exact check has come up empty (T18, T19, T20, T21, T22, T23)** — T19's
Ceremony 1 is still the only one that found a genuine #212/#213-shaped gap in
this section; every scan since (including this one) has confirmed the
section holds no third instance, not merely repeated the prior finding
unchecked.
...
2. **Take the migration-tooling item (or Observability, or the ISO-8601
   ...
   on-the-record roadmap debt, not a disclosed gap, per six separate prior
...
sprint, the fourth in this project's history, stated with its reasoning in
full rather than left implicit.** This ceremony independently re-ran every
check that produced that same conclusion at T20, T21, and T22 (§A3/§A5) and
...
**On whether a fourth consecutive 0-ticket sprint needs its own fresh "is
this healthy" pass: no, and per the task's own explicit instruction this is
not re-litigated from scratch here.** That question was genuinely engaged at
T20 retro and T21 retro, closed by the user's own direct answer, and T21
retro named two concrete conditions under which it should be reopened. T22
retro was the first ceremony to exercise that mechanism — check the named
conditions live rather than re-derive the analysis — and found neither
fired. **This ceremony repeats that same live check, per T22 retro's
recommendation 2, rather than repeating the underlying analysis a fourth
time:**

1. **A materially different blocker profile.** No ninth issue joined D1's
   cluster (still exactly #144/#149/#124), and none of the five
   ...
   against each issue's own blocker text this ceremony, not assumed from
   memory (§A3).
2. **The backlog running dry entirely.** `totalCount: 8`, the identical
   eight issues, none closed, none replaced. It has not moved at all, let
   alone run dry.

**Neither condition holds.** Padding this ceremony's document with a fourth
repeat of the underlying "is a 0-ticket sprint healthy" analysis would
itself be the wrong outcome per the recommendation's own instruction, so
this section states the check and its result rather than re-arguing the
question. Per recommendation 3 (§A2), the precise counts are carried forward
rather than re-derived: the backlog is static across the five most recent
live checks (T21 Ceremony 1, T21 retro, T22 Ceremony 1, T22 retro, this
ceremony), and D1 has now sat with a single unchanging comment for ten
consecutive sprints (T14 through T23) — see §A7.
...
comment, T14.3's original escalation, unchanged across T14 through T22, now
T23. Both ADR-0015's and ADR-0016's own `## Status` fields are unchanged,
read in full this ceremony.

> **DECISION D1 (for the user / Product Owner) — eleventh deferral.** When
...
> four options with their costs and recommends none. No T23 ticket exists to
> implement this (there are no T23 tickets at all), and per ADR-0015's
> restriction list no T23 PR may guess at it regardless. Its footprint is
> unchanged from T22's own re-check: still the same two named instances of
> shaped scope (`CancelBooking`'s own missing check; the court-Bookings half
> of #124), neither grown nor shrunk. Per T22 retro's own framing, D1 has
> now carried its single T14.3 comment for **ten consecutive sprints (T14
> through T23)** with no second escalation attempt beyond the original ADR
> text — the precise, re-checked count carried forward per recommendation 3,
> not re-derived from scratch. Per T21 retro's recommendation 2 (as executed
> at T22), the escalation-mechanism question this deferral's length once
> raised has already been put to the user and answered (keep re-deferring);
> this ceremony does not re-raise it, only names the count honestly.

> **DECISION D2 (for the user) — eighth deferral, ninth sprint open.** When
...
> lays out four options and recommends none. T22 shipped no reviewer-authored
> gap-fix (there were no PRs at all this sprint to test the interim rule
> against) — its third consecutive sprint of that structurally weaker
...
> **T23 also has no tickets**, so this sprint is a fourth instance of that
> same weaker null result, not a ninth instance of T15–T19's stronger
...
|---|---|---|
| **`HANDOFF.md`** | **this ceremony only** | An implementer that finds a stale line flags it for T24's Ceremony 1 and does not edit it — the standing rule, unchanged. No execution ticket exists this sprint to collide with it |
...
by construction.** T23 has zero tickets. There is no wave to have a
collision within.
...
> than re-litigating the underlying question.** All 8 open issues were
> independently re-verified live and found exactly as blocked as T22 left
...
> re-read in full for a sixth sprint running (T18–T23) and produced no third
> #212/#213-shaped find. The `golang-migrate`/`goose` roadmap-debt
> classification remains settled on no new fact. Neither of T21 retro's two
> reopening conditions fired, checked live rather than assumed — the second
> consecutive sprint this specific check has been performed and scored.

**What this sprint does not claim** (the half PM insists on):

- **This is not evidence the project has run out of real work**, and this
  plan does not claim that. A fourth zero-ticket sprint running is the
...
- **This does not mean the 8 tracked issues, or the roadmap-debt items,
  ...
  correctly-unticketed) as it was at T22.
...
six prior ceremonies' own explicit ruling that this ceremony found no new
fact to overturn (§A3).
...
2. **The merged-fix issue sweep run and reported with its count** — by the
   retro (reporting, not blocking) and again by T24's Ceremony 1
...
3. **Scoring owed at the retro:**
   ...
   - **(d)** Did either of T21 retro's two named reopening conditions fire
     ...
     running dry entirely)? Scored live again per T22 retro's
     recommendation 2 — not assumed from this ceremony's own "neither
     fired" result (§A6).
4. **Not scoreable by T23 and deliberately not pre-empted:** D1 and D2
   remain the user's own decisions to make, on whatever timeline the user
   chooses.
5. Retro in `docs/process/t23-retro.md`, indexed by a `## T23 sprint retro`
   stub in `docs/LESSONS.md`. `HANDOFF.md`/`CLAUDE.md` state updated —
   noting that **T24's Ceremony 1**, not the retro, corrects T23's
