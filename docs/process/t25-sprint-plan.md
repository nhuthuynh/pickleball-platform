# T25 Sprint Plan — Ceremonies 1 + 2
...
`docs/process/t24-retro.md` (PR #227, merged `2026-08-16T08:16:42Z`),
...
GitHub API during this ceremony**, at the merged branch tip (`da44fb0`, T24
retro's own merge), rather than taken from the retro's or T24's plan's prose
— CLAUDE.md rule 10 applied to planning. Per the task's own instruction and
the pattern T20–T24's ceremonies established, this ceremony does **not**
...
exhausted across T20–T24's retros. It checks live whether either of T21
...
## §A0 — Correcting T24's Docs-index row and Task-backlog narrative: this
...
`docs/process/t24-retro.md`'s own closing line — *"T25's Ceremony 1 corrects
it, including the honest-form sentence above, as its first job — the same
standing convention every prior ceremony has followed"* — **performed
...
|---|---|---|
| Retro link | `docs/process/t24-retro.md` (no incident-grade finding; independently re-verified live that all 8 issues' blockers held, byte-for-byte against the plan's own table, and that the migration-tooling classification stayed unticketed; scored DoD (d) live for the fifth time running with an identical result; found a recurring citation imprecision — prior retros had quoted ADR-0015/0016's frontmatter status bullet under the "`## Status` field" label rather than the actual `## Status`-headed section a few lines below it — corrected, no DoD score moved; 4 recommendations for T25) | File exists, read in full this ceremony; its own characterization matches its content |
| Reviews cell | PRs #226 (Ceremony 1/2 doc) → #227 (retro doc), in that merge order | Re-fetched both via `list_pull_requests`: `merged_at` **08:11:09Z → 08:16:42Z** — ascending, matching the retro's own dispatch-then-retro order |
| Task-backlog narrative | The retro's own agreed sentence, quoted verbatim from `docs/process/t24-retro.md`'s "The sprint goal, scored" section, per `sprint-process.md` Ceremony 1 item 3 ("the retro's form, not a stronger one") | Cross-checked word-for-word against the retro's own blockquote — copied, not paraphrased |

**A new T25 row is added** in the same before-the-sprint form, Retro and
Reviews honestly marked "not yet written" / "not yet opened", for T26's
Ceremony 1 to correct in turn — the same convention every prior ceremony has
followed.

**The ADR-status citation correction, applied going forward.** Per this
ceremony's own instruction and T24 retro's finding 4: quoting either ADR's
status below uses the text under the actual `## Status` heading, not the
frontmatter bullet a few lines above it. Re-read directly this ceremony
(`grep -n "^## Status" -A 5` on both files):
...
substantive act (per DoD, T25's Ceremony 1 is the authoritative moment)

**Step 1 — list the open issues, live.** `list_issues(state: OPEN)` at
ceremony start → **`totalCount: 8`**: #124, #126, #130, #134, #144, #145,
#149, #164. Identical set to T24's own Ceremony 1 sweep and T24's retro
sweep before it.

**Step 2 — reconcile arithmetically, before reading any issue
individually.** `open_at_end_of_T24's_own_retro − closed_since_then +
opened_since_then`. T24's own retro (its sweep moment 1,
`docs/process/t24-retro.md` finding 1) left the count at **8**. Since that
retro's own PR (#227) merged, zero PRs have merged (confirmed by `git log`,
...
sort: updated, direction: desc)` → the most recent entry is **#227 itself**
(T24's own retro doc, `merged_at: 2026-08-16T08:16:42Z`); nothing merged
after it. `git log --oneline -3` at this ceremony's branch cut shows
`da44fb0` (PR #227's merge, "docs: T24 sprint retro (Ceremony 3)") as the
tip with no descendants, and `git status`/`git fetch` were clean before this
ceremony's branch was cut (no isolated worktree was needed — the shared
checkout was already at the real tip). **Zero PRs to cross-reference against
the open list** — the correct, trivial shape for a sprint whose predecessor
shipped no tickets, checked rather than assumed from the retro's own
prediction.

**Sweep result: clean, continuing the unbroken run since T15.** **T26's
...
## §A2 — T24 retro's four recommendations, each given a disposition

| # | Recommendation | Disposition |
|---|---|---|
| 1 | Continue treating the merged-fix sweep as authoritative regardless of the prior retro's clean result — T25's Ceremony 1 re-runs the sweep and re-verifies the open count from the API rather than trusting the retro's table | **Executed here** (§A1) — the open count was independently re-derived from the live API and reconciled arithmetically, not read off the retro's own table |
| 2 | Keep DoD (d) at the abbreviated one-line-plus-two-sub-checks length — the mechanism has now produced an identical correct result five times running; nothing about a sixth identical result would justify expanding it back out | **Executed here, in the kept-short form** (§A5) — both conditions re-checked against freshly-fetched facts this ceremony, reported in one line plus its two sub-checks. **Neither fired** |
| 3 | Carry forward the two running counts using the distinct shapes finding 6 uses — the backlog's consecutive-static-check count increments every ceremony (now 8); D1's consecutive-sprint-silence count increments only when the sprint number advances (stays at 11 until T25, becomes 12 only if T25 opens with #144 still unanswered) | **Executed here** (§A3/§A7) — both counts independently re-verified as unchanged this ceremony and incremented correctly: the backlog is now static across the **nine** most recent live checks; D1's single T14.3 comment has stood for **twelve** consecutive sprints (T14 through T25), confirmed by this ceremony re-fetching #144 and finding exactly one comment still |
| 4 | If a sixth consecutive 0-ticket sprint arrives, do not open a new "is this healthy" engagement question by default — reserve engagement for a condition actually firing or a direct user instruction | **Executed here** (§A6) — neither condition fired, so this ceremony states that result plainly rather than opening a fresh engagement question |
...
| Every one of the 8 open issues' blockers are unchanged since T24's retro | `list_issues` with comment counts and `updated_at`, live; cross-checked against `docs/process/t24-retro.md`'s own per-issue table | #144 (D1) still exactly 1 comment, `updated_at 2026-08-15T07:01:03Z`; #149 still 3 comments, `2026-08-15T16:56:58Z`; #164 still 1 comment, `2026-08-15T14:16:28Z`; #124 still 1 comment, `2026-08-15T16:25:34Z`; #145 still 1 comment, `2026-08-15T05:01:29Z`; #126/#130/#134 still 0 comments, `2026-08-14T16:12:26Z`/`2026-08-14T16:30:25Z`/`2026-08-14T16:37:49Z`. **Every field matches the retro's own live-fetched table exactly, byte-for-byte — zero blockers changed state, zero new comments on any of the 8** |
| D1 (#144) still has exactly one comment | live fetch (`list_issues` comment count for #144) | confirmed — T14.3's original escalation only, unchanged across T14 through T24, now T25 (twelfth sprint) |
| D1 (ADR-0015) still unanswered | read `docs/adr/0015-*.md`'s `## Status` heading in full (not the frontmatter bullet — §A0) | unchanged: **"Escalated — awaiting product decision. This ADR decides nothing."** No option (a)–(d) marked chosen anywhere in the document |
...
| No PR merged since T24's retro's own PR | `git fetch`, `git log --oneline -3`, `git status`, `list_pull_requests` most-recent entry | clean, up to date with `origin/claude/go-backend-pickleball-7up34j`, HEAD at `da44fb0` (T24 retro's own merge) before this ceremony's branch was cut; most recent merged PR against the shared branch is #227 itself |
| `HANDOFF.md`'s Cross-cutting section re-read for any new disclosed-but-never-filed gap — the eighth sprint running this exact check (T18–T24, now T25) | Read the entire Cross-cutting section (`HANDOFF.md`, the full block between "## Cross-cutting / later" and "## Definition of Done", ~665 lines) directly, plus a targeted grep for the phrases prior gaps have carried (`raise at the next backlog refinement`, `follow-up ticket`, `not yet ticketed`, `deferred with no ticket`, `logged not fixed`) to cross-check nothing was skimmed past | **No new candidate of #212/#213's shape found.** Every "follow-up ticket"/"logged not fixed" hit is one of: already closed (T5.2's `Game.Status` gap → #212/T19.1; T6.4's concurrency-proof gap → #213/T19.2; the `RefundPayment` wiring gap → T12.3; the waitlist-position race → T6.6-loop-2; the Competitions-shaped `PayableType` gap → T10.6/#96), or already tracked as one of the 8 open issues (T8.10's price-field gap → #126). T19's Ceremony 1 remains the only one that ever found a genuine #212/#213-shaped gap here; this is the eighth re-scan since (T20–T24, now T25) to confirm the section holds no third instance |
| The `golang-migrate`/`goose` migration-tooling swap, checked against this project's own settled precedent, not treated as new | `grep -rn "golang-migrate\|goose" HANDOFF.md docs/process/*.md docs/adr/*.md`, then re-read the T11–T14/T21–T24 sprint plans' own dispositions | Same result as T20's through T24's own checks: eight separate prior ceremonies (T11–T14, T21–T24) already and explicitly ruled this out of the mandatory-issue treatment — "roadmap items, not disclosed gaps." No new fact surfaced this sprint (zero PRs merged) that could overturn that classification. **Disposition: still correctly unticketed, unchanged** |
...
| `make gate-coverage` still green | ran directly | `OK — all 42 package(s) with runnable tests are executed by "ci-checks"` — unchanged from T24, no new package landed since no PR merged |
...
| Local worktree matches the shared branch's real tip | `git fetch`, `git status`, `git log --oneline -3` | clean, up to date with `origin/claude/go-backend-pickleball-7up34j`, HEAD at `da44fb0` before this ceremony's branch was cut |
...
No T25 ticket exists (§A6), so no ticket names a database table or a
...
|---|---|---|
| **#144** `CancelBooking`/`CreateBooking` have no authz | **Escalated** | D1 unanswered; §A7. Thirteenth sprint carrying this |
...
| **#134** WCAG manual screen-reader pass | **Deferred** | Needs a `role:ux-ui-designer` pass with real assistive-technology hardware this environment cannot provide; T25 ships no UI change |

**Every one of the 8 tracked issues remains genuinely blocked, independently
re-verified rather than assumed** (§A3) — the fourteenth consecutive time
this exact set of blockers has been re-checked and found unchanged (T12
through T25). Zero of the 8 open issues are actionable this sprint without
...
found nothing new to promote into an issue — the eighth sprint running this
exact check has come up empty (T18–T24, now T25)** — T19's Ceremony 1 is
...
and T24 retro's recommendation 2 ask for (a one-line result plus its two
sub-checks, not a dedicated finding) — neither of T21 retro's two named
reopening conditions fired:**
...
2. **Take the migration-tooling item (or Observability, or the ISO-8601
   ...
   on-the-record roadmap debt, not a disclosed gap, per eight separate prior
...
sprint, the sixth in this project's history, stated with its reasoning in
full rather than left implicit.** This ceremony independently re-ran every
check that produced that same conclusion at T20 through T24 (§A3/§A5) and
...
**On whether a sixth consecutive 0-ticket sprint needs its own fresh "is
...
sprints; T24's retro independently re-ran the same live check and found the
same result a fifth time; this ceremony is the sixth independent run of that
mechanism, on freshly-fetched facts, and it too found neither condition
fired. Per T24 retro's own recommendation 4 — *"if a sixth consecutive
0-ticket sprint arrives, do not open a new 'is this healthy' engagement
question by default"* — this section states the check and its result rather
than opening a fresh analysis.
...
comment, T14.3's original escalation, unchanged across T14 through T24, now
T25. Both ADR-0015's and ADR-0016's own `## Status` headings are unchanged,
read in full this ceremony (§A0).

> **DECISION D1 (for the user / Product Owner) — thirteenth deferral.** When
...
> four options with their costs and recommends none. No T25 ticket exists to
> implement this (there are no T25 tickets at all), and per ADR-0015's
> restriction list no T25 PR may guess at it regardless. Its footprint is
> unchanged from T24's own re-check: still the same two named instances of
...
> comment for **twelve consecutive sprints (T14 through T25)** with no
> second escalation attempt beyond the original ADR text — the precise,
> re-checked count carried forward per T24 retro's recommendation 3, not
...
> **DECISION D2 (for the user) — tenth deferral, eleventh sprint open.** When
...
> lays out four options and recommends none. T24 shipped no reviewer-authored
> gap-fix (there were no PRs at all this sprint to test the interim rule
> against) — its sixth consecutive sprint of that structurally weaker
...
> **T25 also has no tickets**, so this sprint is a **seventh** instance of
> that same weaker null result (T20 through T25), bringing the combined
> total to eleven sprints (T15 through T25) of D2 sitting open — not an
> eleventh instance of T15–T19's stronger one, named precisely rather than
...
|---|---|---|
| **`HANDOFF.md`** | **this ceremony only** | An implementer that finds a stale line flags it for T26's Ceremony 1 and does not edit it — the standing rule, unchanged. No execution ticket exists this sprint to collide with it |
...
by construction.** T25 has zero tickets. There is no wave to have a
collision within.
...
> **Correct the record for T24, confirm rather than assume that the tracked
...
> conditions live, every sprint, in the abbreviated form T23/T24 retro's
> recommendations ask for, rather than re-litigating the underlying question
> at length.** All 8 open issues were independently re-verified live and
> found exactly as blocked as T24 left them: three on DECISION D1, two on a
> real IdP tenant this environment cannot provision, two on genuine Product
> Owner input, one on real assistive-technology hardware. `HANDOFF.md`'s
> Cross-cutting section was re-read in full for an eighth sprint running
> (T18–T25) and produced no third #212/#213-shaped find. The
...
> live rather than assumed — the sixth consecutive time this specific check
...
- **This is not evidence the project has run out of real work**, and this
  plan does not claim that. A sixth zero-ticket sprint running is the
...
- **This does not mean the 8 tracked issues, or the roadmap-debt items,
  ...
  correctly-unticketed) as it was at T24.
...
eight prior ceremonies' own explicit ruling that this ceremony found no new
fact to overturn (§A3).
...
2. **The merged-fix issue sweep run and reported with its count** — by the
   retro (reporting, not blocking) and again by T26's Ceremony 1
...
3. **Scoring owed at the retro:**
   ...
   - **(d)** Did either of T21 retro's two named reopening conditions fire
     ...
     form T23/T24 retro's recommendations established — not a dedicated
     finding unless a condition actually fires.
4. **Not scoreable by T25 and deliberately not pre-empted:** D1 and D2
   remain the user's own decisions to make, on whatever timeline the user
   chooses.
5. Retro in `docs/process/t25-retro.md`, indexed by a `## T25 sprint retro`
   stub in `docs/LESSONS.md`. `HANDOFF.md`/`CLAUDE.md` state updated —
   noting that **T26's Ceremony 1**, not the retro, corrects T25's
