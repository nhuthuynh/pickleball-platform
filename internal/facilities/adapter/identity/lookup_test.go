// Package identity_test covers internal/facilities/adapter/identity, the one
// place Facilities is allowed to call into the Identity context (T13.3).
//
...
// the entire point of this file, and it is the shape T13.1 established for
// cross-context adapters: issue #154 shipped because Facilities had no
// identity boundary at all, so the actual behaviour of resolving a real,
// non-uuid IdP subject was never exercised anywhere in this context — and a
// subject is the only actor value production has carried since T12.7 made the
// actor auth.RequireSubject(ctx).
...
// provider string, and that is exactly why it can never be stored in
// facilities.owner_id, and why this adapter has to exist.
const fixtureSubject = "auth0|abc123"

// fixtureUserID is the server-minted uuid primary key. Since T12.9 the ID and
// the Subject are two different identifier spaces (ADR-0014), which is the
// whole subject of issue #154.
...
// newLookup builds the adapter over a REAL identityapp.Service and returns it
// alongside the repo fake, so a test can both seed and fail the upstream.
func newLookup(t *testing.T) (*facilitiesidentity.Lookup, *inMemoryRepo) {
	t.Helper()

	repo := newInMemoryRepo()
	return facilitiesidentity.NewLookup(identityapp.NewService(repo, stubIDs{})), repo
}

// seedUser writes a User straight through the repository fake rather than
// through identityapp.Service.CreateUser, so the test controls which uuid the
// subject resolves to and can assert on it.
...
// headline behaviour: the translation ADR-0014 rules must happen, proven
// against the real identityapp.Service rather than a fake of it.
//
// The two assertions are separate on purpose. "No error" alone would pass for
// an adapter that returned the subject back unchanged — which is precisely
// the pre-T13.3 behaviour of the code path this replaces.
func TestUserIDBySubject_ResolvesARealSubjectToTheUserID(t *testing.T) {
	...
	if got == fixtureSubject {
		t.Fatalf("UserIDBySubject(%q) returned the subject unchanged — no translation "+
			"happened, which is issue #154", fixtureSubject)
	}
...
// TestUserIDBySubject_UnknownSubjectIsFacilitiesErrUserNotFound pins CLAUDE.md
// rule 5 at this boundary: the caller gets Facilities' OWN sentinel, and the
// Identity one does not leak through.
//
// The negative assertion is the one that matters. Checking only
// errors.Is(err, facilitiesdomain.ErrUserNotFound) would still pass if the
// adapter wrapped identitydomain.ErrUserNotFound with %w, letting app-layer
// code start matching on another context's error type — which is exactly the
// leak rule 5 forbids.
func TestUserIDBySubject_UnknownSubjectIsFacilitiesErrUserNotFound(t *testing.T) {
	t.Parallel()

	lookup, _ := newLookup(t)

	_, err := lookup.UserIDBySubject(context.Background(), "auth0|never-registered")
	if !errors.Is(err, facilitiesdomain.ErrUserNotFound) {
		t.Fatalf("UserIDBySubject(unknown) = %v, want facilities' own ErrUserNotFound", err)
	}
...
// TestUserIDBySubject_OtherErrorsAreWrappedWithoutTheirType covers translate's
// default branch: an infrastructure failure that is not "no such user".
//
// It must still not cross the boundary as its original type (the adapter uses
// %s, not %w), because a Facilities-side caller must never be able to
...
// port.IdentityLookup's doc comment makes, pinned so a future method addition
// has to confront it deliberately rather than by accident.
//
// The port exists to let a uuid cross the context boundary and nothing else.
// A method returning identitydomain.User would let Facilities' app layer
// start reading Roles or SelfReportedStartingLevel and quietly acquire a
// dependency on another context's model.
func TestLookup_ExposesNoUserReturningMethod(t *testing.T) {
	...
	// ever added to the port, this file's other tests keep passing — but the
	// reviewer reading this one is told what the port promised.
	var _ interface {
		UserIDBySubject(ctx context.Context, subject string) (string, error)
	} = (*facilitiesidentity.Lookup)(nil)
}
