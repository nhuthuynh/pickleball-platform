// Package identity_test covers internal/payments/adapter/identity, the one
// place Payments is allowed to call into the Identity context (T28.1).
//
...
// internal/facilities/adapter/identity/lookup_test.go established (T13.3)
...
// it. A subject is the only actor value Payments' actor(ctx) funnel has ever
// carried, since T12.8.
...
// payments.recorded_by_user_id once that column is a uuid, and why this
...
// the whole subject of #164's Payments third.
...
func newLookup(t *testing.T) (*paymentsidentity.Lookup, *inMemoryRepo) {
	t.Helper()

	repo := newInMemoryRepo()
	return paymentsidentity.NewLookup(identityapp.NewService(repo, stubIDs{})), repo
}
...
// precisely the pre-T28.1 behaviour of the code path this replaces
...
func TestUserIDBySubject_ResolvesARealSubjectToTheUserID(t *testing.T) {
	...
	if got == fixtureSubject {
		t.Fatalf("UserIDBySubject(%q) returned the subject unchanged — no translation "+
			"happened, which is #164's Payments third", fixtureSubject)
	}
...
// TestUserIDBySubject_UnknownSubjectIsPaymentsErrUserNotFound pins CLAUDE.md
// rule 5 at this boundary: the caller gets Payments' OWN sentinel, and the
// Identity one does not leak through.
//
// The negative assertion is the one that matters. Checking only
// errors.Is(err, paymentsdomain.ErrUserNotFound) would still pass if the
...
func TestUserIDBySubject_UnknownSubjectIsPaymentsErrUserNotFound(t *testing.T) {
	t.Parallel()

	lookup, _ := newLookup(t)

	_, err := lookup.UserIDBySubject(context.Background(), "auth0|never-registered")
	if !errors.Is(err, paymentsdomain.ErrUserNotFound) {
		t.Fatalf("UserIDBySubject(unknown) = %v, want payments' own ErrUserNotFound", err)
	}
...
// uses %s, not %w), because a Payments-side caller must never be able to
...
// else. A method returning identitydomain.User would let Payments' app
