...
// the one T13.9's payments template uses throughout. But Competitions' domain
// declares two sentinels that only the Payments-driven port can produce — see
// the "deliberately unreachable" rows in that file — and a table that silently
...
// TestCompetitionsService_HasExactlyOneErrorMapper enforces that "one".
