// Package identity_test covers internal/competitions/adapter/identity, the
// one place Competitions is allowed to call into the Identity context
// (T29.1).
//
...
// internal/facilities/adapter/identity/lookup_test.go (T13.3) and
// internal/payments/adapter/identity/lookup_test.go (T28.1) established and
// this file mirrors exactly, because the entire point is exercising the
// actual behaviour of resolving a real, non-uuid IdP subject, not a fake of
// it. A subject is the only actor value Competitions' actor(ctx) has ever
// carried, since T12.8.
...
// competitions.host_id (etc.) once those columns are uuid, and why this
// adapter has to exist.
...
// the whole subject of #164's Competitions third.
...
func newLookup(t *testing.T) (*competitionsidentity.Lookup, *inMemoryRepo) {
	t.Helper()

	repo := newInMemoryRepo()
	return competitionsidentity.NewLookup(identityapp.NewService(repo, stubIDs{})), repo
}
...
// precisely the pre-T29.1 behaviour of the code path this replaces
...
func TestUserIDBySubject_ResolvesARealSubjectToTheUserID(t *testing.T) {
	...
	if got == fixtureSubject {
		t.Fatalf("UserIDBySubject(%q) returned the subject unchanged — no translation "+
			"happened, which is #164's Competitions third", fixtureSubject)
	}
...
// TestUserIDBySubject_UnknownSubjectIsCompetitionsErrUserNotFound pins
// CLAUDE.md rule 5 at this boundary: the caller gets Competitions' OWN
// sentinel, and the Identity one does not leak through.
//
// The negative assertion is the one that matters. Checking only
// errors.Is(err, competitionsdomain.ErrUserNotFound) would still pass if the
...
func TestUserIDBySubject_UnknownSubjectIsCompetitionsErrUserNotFound(t *testing.T) {
	t.Parallel()

	lookup, _ := newLookup(t)

	_, err := lookup.UserIDBySubject(context.Background(), "auth0|never-registered")
	if !errors.Is(err, competitionsdomain.ErrUserNotFound) {
		t.Fatalf("UserIDBySubject(unknown) = %v, want competitions' own ErrUserNotFound", err)
	}
...
// uses %s, not %w), because a Competitions-side caller must never be able to
...
// else. A method returning identitydomain.User would let Competitions' app
...
func TestLookup_ExposesNoUserReturningMethod(t *testing.T) {
	...
	} = (*competitionsidentity.Lookup)(nil)
}
